...
class Trainer:
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            # Use smaller batch size for eval if needed, get from config or default
            eval_batch_size = self.config.get('eval_batch_size', self.config['batch_size'])
            hidden_test = self.model.init_hidden(eval_batch_size) # Init hidden state
            ...
            for k in range(eval_iters):
                try:
                    ...
                    # Use current E (detached) for evaluation embeddings
                    test_embeddings = F.embedding(x_test, self.E.detach())

                    test_logits, hidden_test = self.model(test_embeddings, hidden_test)
                    hidden_test = tuple([h.detach() for h in hidden_test]) # Detach hidden state
                    ...
                    # Break or continue? Continue but log maybe.
                    continue # Try next batch

        self.model.train() # Set model back to training mode
        avg_loss = total_loss / actual_eval_iters if actual_eval_iters > 0 else float('nan')
